...
describe('already-closed fd', () => {
  ...
  it('guard.drop() rejects but still marks guard as dropped', async () => {
    ...
    const guard = await openLock(file, Lock.Exclusive, { fs: noGcFs })
...
describe('hook state after unlock failure', () => {
  ...
  it('unregister cleans up hook state after guard.drop() with closed fd', async () => {
    ...
    const h = await fs.promises.open(getPath('hook-cleanup'), fs.constants.O_CREAT | fs.constants.O_RDWR)
    await locker.lock(h.fd, Lock.Exclusive)
    expect(activeCount).toBe(1)

    // Close fd to force unlock failure
    fs.closeSync(h.fd)
    await locker.unlock(h.fd).catch(() => {})
